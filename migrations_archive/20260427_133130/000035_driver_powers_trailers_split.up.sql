...
-- Migrate data and drop columns only if drivers still has power_plate_number (idempotent: skip if already split)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'drivers' AND column_name = 'power_plate_number'
  ) THEN
