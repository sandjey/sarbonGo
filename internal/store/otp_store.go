...
func (s *OTPStore) SaveOTP(ctx context.Context, phone, code, requestID, ip string) error {
	...
	// OTP save doesn't need atomicity across keys.
	// We run commands sequentially to get precise Redis errors (e.g. WRONGTYPE)
	// instead of the generic Tx MULTI/EXEC abort.
	if err := s.rdb.Del(ctx, key).Err(); err != nil {
		return fmt.Errorf("redis del otp key failed: %w", err)
	}
	// Redis 3.x supports only a single field/value for HSET, but our OTP key needs 3 fields.
	// HMSET is the compatible command for Redis < 4.0.
	if ok, err := s.rdb.HMSet(ctx, key, map[string]interface{}{
		"hash":       s.hash(phone, code),
		"attempts":   "0",
		"request_id": requestID,
	}).Result(); err != nil {
		return fmt.Errorf("redis hmset otp key failed: %w", err)
	} else if !ok {
		return fmt.Errorf("redis hmset otp key failed: result=false")
	}
	if err := s.rdb.Expire(ctx, key, s.ttl).Err(); err != nil {
		return fmt.Errorf("redis expire otp key failed: %w", err)
	}

	if s.cooldown > 0 {
		if err := s.rdb.Del(ctx, s.cooldownKey(phone)).Err(); err != nil {
			return fmt.Errorf("redis del cooldown key failed: %w", err)
		}
		if err := s.rdb.Set(ctx, s.cooldownKey(phone), "1", s.cooldown).Err(); err != nil {
			return fmt.Errorf("redis set cooldown key failed: %w", err)
		}
	}
